...
// Callout component based on variant
function Callout({
  variant,
  text
}: {
      ...
      {items.map((item, index) => {
        ...
        return (
          <div key={index} className="text-center">
            {value && (
              <div className="text-2xl md:text-3xl font-black text-yt-red mb-1">
                {value}
              </div>
            )}
            <div className="text-xs text-dark-ink-secondary uppercase tracking-wide">
              {label}
            </div>
          </div>
        )
...
function QuoteSection({
  ...
  return (
    <blockquote className="p-6 border-l-4 border-yt-red bg-dark-surface/50">
      ...
      <cite className="text-sm text-dark-ink-secondary not-italic">
        — {author}
      </cite>
    </blockquote>
  )
...
function renderSection(section: {
  ...
  switch (section.type) {
    case "intro":
      return (
        <p className="text-lg text-dark-ink-secondary leading-relaxed mb-8">
          {section.content}
        </p>
      )
    ...
    case "h2":
      return (
        <section className="mb-8">
          <h2 className="text-2xl md:text-3xl font-bold mb-4 text-dark-ink">
            {section.heading}
          </h2>
          {section.content && (
            <p className="text-dark-ink-secondary mb-4">{section.content}</p>
          )}
          {section.subsections?.map((sub, i) => (
            <div key={i} className="mb-6">
              <h3 className="text-xl font-semibold mb-2 text-dark-ink">
                {sub.heading}
              </h3>
    ...
    case "h3":
      return (
        <div className="mb-6 pl-4 border-l-2 border-yt-red/30">
          <h3 className="text-lg font-semibold mb-2 text-dark-ink">
            {section.heading}
          </h3>
...
function RelatedArticles({
  ...
  // This would need to fetch articles by IDs - for now we use a workaround
  // In a real implementation, you'd add a getArticlesByIds function to blog.ts
  return null // Placeholder - will show inline cards instead
}
      ...
      <main className="max-w-[1400px] mx-auto px-4 py-12">
        ...
        <article className="mb-12">
          <header className="mb-8">
            ...
            {/* Subtitle */}
            {article.subtitle && (
              <p className="text-xl text-dark-ink-secondary mb-6 max-w-3xl">
                {article.subtitle}
              </p>
            )}
