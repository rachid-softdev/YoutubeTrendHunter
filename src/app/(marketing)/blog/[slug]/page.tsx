...
function Callout({ variant, text }: { variant: string; text: string }) {
  const variants = {
    tip: { icon: Lightbulb, bg: "bg-yt-red/10", border: "border-yt-red/30", iconColor: "text-yt-red" },
      ...
      {items.map((item, index) => {
        ...
        return (
          <div key={index} className="text-center">
            {value && <div className="text-2xl md:text-3xl font-black text-yt-red mb-1">{value}</div>}
...
function renderSection(section: { type: string; heading?: string; content?: string; variant?: string; text?: string; items?: (string | { label?: string; value?: string })[]; author?: string; subsections?: { type?: string; heading?: string; content?: string }[] }) {
  switch (section.type) {
    case "intro":
      return <p className="text-lg text-dark-ink-secondary leading-relaxed mb-8">{section.content}</p>
    case "h2":
      return (
    ...
    case "callout":
      return <div className="mb-8"><Callout variant={section.variant || "info"} text={section.text || ""} /></div>
    case "stats":
      return <div className="mb-8"><StatsSection items={section.items} /></div>
    case "items":
      return (
    ...
    case "quote":
      return <div className="mb-8"><QuoteSection text={section.text || ""} author={section.author || ""} /></div>
    case "conclusion":
      return (
...
export default async function BlogArticlePage({ params }: Props) {
  ...
  const relatedArticles = article.relatedArticles?.map((id) => allArticles.find((a) => a.id === id)).filter(Boolean) || []
      ...
      <main className="max-w-[1400px] mx-auto px-4 py-12">
        <nav className="mb-8">
          <div className="flex items-center gap-2 text-sm">
            <Link href="/" className="text-dark-ink-secondary hover:text-dark-ink transition-colors">Accueil</Link>
            <span className="text-dark-ink-tertiary">/</span>
            <Link href="/blog" className="text-dark-ink-secondary hover:text-dark-ink transition-colors">Blog</Link>
            <span className="text-dark-ink-tertiary">/</span>
        ...
        <article className="mb-12">
          <header className="mb-8">
            <div className="flex flex-wrap gap-2 mb-4">
              <Badge variant="outline" className="uppercase tracking-wide">{article.category}</Badge>
              {content.difficulty && <Badge variant={getDifficultyVariant(content.difficulty)}>{content.difficulty}</Badge>}
            </div>
            <h1 className="text-3xl md:text-4xl lg:text-5xl font-black mb-4 leading-tight">{article.title}</h1>
            {article.subtitle && <p className="text-xl text-dark-ink-secondary mb-6 max-w-3xl">{article.subtitle}</p>}
            <div className="flex flex-wrap items-center gap-4 text-sm text-dark-ink-secondary">
              <div className="flex items-center gap-2"><User className="w-4 h-4" /><span>{article.author.name}</span></div>
              <div className="flex items-center gap-2"><Calendar className="w-4 h-4" /><span>{formatDate(article.timestamps.publishedAt)}</span></div>
              <div className="flex items-center gap-2"><Clock className="w-4 h-4" /><span>{content.readTime} min de lecture</span></div>
            </div>
            {content.tags && content.tags.length > 0 && (
              <div className="flex flex-wrap gap-2 mt-4">
                <Tag className="w-4 h-4 text-dark-ink-tertiary" />
                {content.tags.map((tag) => <Badge key={tag} variant="outline" className="text-xs">{tag}</Badge>)}
              </div>
            )}
          </header>
          <Separator className="mb-8" />
          <div className="prose prose-invert max-w-none">
            {content.sections.map((section, index) => <div key={index}>{renderSection(section)}</div>)}
          </div>
        </article>
                  ...
                  <Link key={related.id} href={`/blog/${related.slug}`}>
                    <Card className="h-full hover:border-yt-red/30 transition-colors group cursor-pointer">
                      <CardContent className="p-6">
                        ...
                        <h3 className="font-bold text-lg mb-2 group-hover:text-yt-red transition-colors line-clamp-2">{related.title}</h3>
                        {related.excerpt && <p className="text-sm text-dark-ink-secondary line-clamp-2">{related.excerpt}</p>}
                        <div className="flex items-center gap-3 mt-4 text-xs text-dark-ink-tertiary">
                          <span className="flex items-center gap-1"><Clock className="w-3 h-3" />{related.content.fr.readTime} min</span>
        ...
        <div className="mt-12 text-center">
          <Link href="/blog" className="inline-flex items-center gap-2 text-dark-ink-secondary hover:text-dark-ink transition-colors">
