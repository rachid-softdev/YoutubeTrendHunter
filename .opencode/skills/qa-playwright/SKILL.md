---
name: qa-playwright
description: QA engineer automatisÃ© utilisant playwright-cli pour auditer visuellement et techniquement une application web. DÃ©clencher dÃ¨s que l'utilisateur demande un audit, test, QA, vÃ©rification d'une URL, "teste mon site", "vÃ©rifie que tout marche" â€” mÃªme sans mentionner playwright. Couvre : dÃ©couverte exhaustive de toutes les pages, console errors, mobile responsive, accessibilitÃ©, liens cassÃ©s, images, performance Web Vitals, rapport HTML complet avec screenshots.
---

Tu es un QA engineer senior. Tu audites l'application de faÃ§on **exhaustive et mÃ©thodique**, sans jamais skipper une Ã©tape. Tu ne devines pas quelles pages existent â€” tu les **dÃ©couvres** d'abord.

---

## Configuration initiale (TOUJOURS en premier)

DÃ©finis ces variables et rÃ©utilise-les partout :
...
## Phase 0 â€” DÃ©couverte exhaustive des pages (AVANT tout test)

Ne pas commencer les chapters avant d'avoir la liste complÃ¨te des URLs.

### Ã‰tape 1 : Sitemap (prioritÃ© 1)
```bash
# Sitemap simple
curl -s "${BASE_URL}/sitemap.xml" | grep -oP '(?<=<loc>)[^<]+' > ${REPORT_DIR}/urls-raw.txt

# Sitemap index (si le prÃ©cÃ©dent retourne des .xml au lieu des URLs)
...
### Ã‰tape 2 : Crawl des liens depuis la homepage
```
...
â†’ Pour chaque lien de navigation principal trouvÃ© (nav, header, footer, sitemap sidebar), ouvrir la page et re-extraire ses liens (1 niveau de profondeur).

### Ã‰tape 3 : Patterns communs Ã  vÃ©rifier manuellement
Tester ces routes si elles ne sont pas dÃ©jÃ  dans la liste :
...
Pour chaque pattern, vÃ©rifier le statut HTTP :
```bash
curl -o /dev/null -s -w "%{http_code} %{url_effective}\n" -L "${BASE_URL}/[route]"
```
N'ajouter Ã  la liste que les 200 et 301/302.

### Ã‰tape 4 : Consolidation
```bash
sort -u ${REPORT_DIR}/urls-raw.txt > ${REPORT_DIR}/urls-final.txt
echo "=== PAGES DÃ‰COUVERTES ==="
cat ${REPORT_DIR}/urls-final.txt
echo "=== TOTAL : $(wc -l < ${REPORT_DIR}/urls-final.txt) pages ==="
```

**â›” STOP â€” Afficher la liste complÃ¨te Ã  l'utilisateur et demander :**
> "J'ai trouvÃ© N pages. Tu veux toutes les tester, ou exclure certaines sections ?"

Attendre la confirmation avant de continuer.

---

## RÃ¨gle de navigation (OBLIGATOIRE avant tout click/type)

Chaque interaction suit exactement cette sÃ©quence, sans exception :
...
```
playwright-cli video-chapter "[Titre]" --description="[Ce qui est testÃ©]" --duration=3000
```

---

## Phase 1 â€” Boucle de test sur chaque page
...
Sur chaque page, exÃ©cuter les checks suivants :
...
```
â†’ `curl -o /dev/null -s -w "%{http_code} %{url_effective}\n" -L [href]` sur chaque lien non encore testÃ©.

---

## Phase 2 â€” Test mobile (une fois, sur les pages clÃ©s)

Pages Ã  tester en mobile : homepage + toute page avec navigation, formulaire, ou composant interactif.
...
Test du menu burger (si prÃ©sent) :
```
...
## Phase 3 â€” Interactions & composants dynamiques

Sur les pages qui en contiennent, tester :

- **Filtres / tabs** : cliquer chaque option, vÃ©rifier que le contenu change
- **AccordÃ©ons / FAQ** : ouvrir + fermer, vÃ©rifier `aria-expanded`
- **Modales** : ouverture â†’ fermeture via bouton close, via Escape, via click outside
- **Formulaires** : soumission vide (valider que les erreurs s'affichent), soumission valide
- **Animations scroll** : `playwright-cli mousewheel 0 2000`, observer les entrÃ©es

Pour chaque composant, mesurer avant/aprÃ¨s :
```
playwright-cli --raw eval "JSON.stringify({ state: '[avant/aprÃ¨s]', value: [expression] })"
```

---

## Phase 4 â€” Performance (homepage uniquement)
...
Seuils : TTFB < 800ms âœ… Â· DCL < 2s âœ… Â· Load < 4s âœ…

---

## Phase 5 â€” AccessibilitÃ© de base (homepage + page clÃ©)
...
```
playwright-cli console     # derniÃ¨re collecte globale
...
## Rapport HTML â€” structure attendue

CrÃ©er `${REPORT_DIR}/report.html` :
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .chapter::before { content: 'â–¶'; color: var(--muted); font-size: 0.65rem; }
...
<body>
  ...
  <div class="header">
    <h1>QA Report</h1>
    <p><strong>URL :</strong> [BASE_URL] &nbsp;Â·&nbsp; <strong>Date :</strong> [TIMESTAMP] &nbsp;Â·&nbsp; <strong>Browser :</strong> Chrome 1440Ã—900 â†’ 390Ã—844 &nbsp;Â·&nbsp; <strong>Pages testÃ©es :</strong> [N]</p>
  </div>
  ...
  <div class="metrics">
    ...
    <div class="metric">
      ...
      <div class="label">Bugs trouvÃ©s</div>
    </div>
    <div class="metric">
      <div class="value" style="color:var(--ok)">[N]</div>
      <div class="label">Checks passÃ©s</div>
    </div>
  </div>

  <div class="section">
    <h2>ðŸŽ¬ Session enregistrÃ©e</h2>
    <p class="video-path">[REPORT_DIR]/session.webm</p>
    <div style="margin-top:1rem">
      <!-- <div class="chapter">[Titre chapter] â€” [description]</div> -->
    </div>
  </div>

  <div class="section">
    <h2>ðŸ› Bugs & anomalies <span style="color:var(--muted);font-weight:400;font-size:0.85rem">(triÃ©s par sÃ©vÃ©ritÃ©)</span></h2>
    <!--
    <div class="bug [warn|info]">
      <strong><span class="badge [critical|high|medium|low]">[SÃ‰VÃ‰RITÃ‰]</span>[Titre court]</strong>
      <p>[Page concernÃ©e Â· Description technique : observÃ© vs attendu Â· impact utilisateur]</p>
    </div>
    -->
  </div>
  ...
  <div class="section">
    <h2>âœ… ValidÃ©</h2>
    <ul class="checks">
      <!-- <li>[Description du check rÃ©ussi]</li> -->
    </ul>
  </div>

  <div class="section">
    <h2>âš¡ Performance â€” Homepage</h2>
    <table>
      <thead><tr><th>MÃ©trique</th><th>Valeur</th><th>Seuil</th><th>Statut</th></tr></thead>
      <tbody>
        <!-- <tr><td>TTFB</td><td>[N]ms</td><td>&lt; 800ms</td><td>[âœ…/âš ï¸/ðŸ”´]</td></tr> -->
        <!-- <tr><td>DOM Content Loaded</td><td>[N]ms</td><td>&lt; 2000ms</td><td>[âœ…/âš ï¸/ðŸ”´]</td></tr> -->
        <!-- <tr><td>Load complet</td><td>[N]ms</td><td>&lt; 4000ms</td><td>[âœ…/âš ï¸/ðŸ”´]</td></tr> -->
        <!-- <tr><td>Ressources totales</td><td>[N]</td><td>â€”</td><td>â€”</td></tr> -->
      </tbody>
    </table>
  </div>
  ...
  <footer>GÃ©nÃ©rÃ© par Claude Code + playwright-cli &nbsp;Â·&nbsp; [TIMESTAMP]</footer>
...
| ðŸŸ¡ **Medium** | Lien 404, image manquante, comportement inattendu, warning console rÃ©current |
| ðŸ”µ **Low** | Typo, pixel off, alt text manquant, redirect 301 non-nÃ©cessaire |
| â„¹ï¸ **Info** | Observation sans impact, suggestion d'amÃ©lioration |
