...
export const TrendCard = memo(
  function TrendCard({ trend, source = "dashboard" }: TrendCardProps) {
    ...
    const handleClick = useCallback(() => {
      analytics.trendViewed(trend.id, trend.niche?.slug || "unknown", trend.score, source);
    ...
    const VelocityIcon =
      trend.velocity > 0 ? TrendingUp : trend.velocity < 0 ? TrendingDown : Minus;
    ...
    return (
      ...
      >
        <Card
          className={`transition-all duration-300 hover:shadow-lg group border-hairline-dark ${isHot ? "border-yt-red/30" : "hover:border-dark-ink-tertiary"} rounded-none`}
        >
          <CardContent className="flex items-start gap-3 p-4">
            ...
            <div className="flex-1 min-w-0">
              ...
              {trend.description && (
                <p className="text-sm text-dark-ink-secondary mt-1 line-clamp-2">
                  {trend.description}
                </p>
              )}
              ...
              <div className="flex items-center gap-3 mt-2 text-sm text-dark-ink-secondary">
                ...
                {trend.videoCount && <span>{trend.videoCount} vidéos</span>}
              </div>
            ...
            <Badge variant={statusVariant}>{trend.status}</Badge>
          </CardContent>
        </Card>
      </div>
    );
