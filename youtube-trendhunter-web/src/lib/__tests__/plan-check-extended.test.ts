...
describe("isOnTrial", () => {
  ...
  it("returns false when no trial dates", async () => {
    vi.mocked(prisma.subscription.findUnique).mockResolvedValue(
      makeSubscription({ trialEnd: null, trialStart: null }),
    );
    const result = await isOnTrial("user_123");
    expect(result).toBe(false);
  });
  ...
  it("returns false when trial has not started", async () => {
    ...
    vi.mocked(prisma.subscription.findUnique).mockResolvedValue(
      makeSubscription({ trialEnd: futureEnd, trialStart: futureStart }),
    );
    const result = await isOnTrial("user_123");
    expect(result).toBe(false);
  });
  ...
  it("returns true when trial is active", async () => {
    ...
    vi.mocked(prisma.subscription.findUnique).mockResolvedValue(
      makeSubscription({ trialEnd: futureEnd, trialStart: pastStart }),
    );
    const result = await isOnTrial("user_123");
  ...
  it("returns false when trial has ended", async () => {
    ...
    vi.mocked(prisma.subscription.findUnique).mockResolvedValue(
      makeSubscription({ trialEnd: pastEnd, trialStart: pastStart }),
    );
    const result = await isOnTrial("user_123");
    expect(result).toBe(false);
  });
...
describe("getTrialDaysRemaining", () => {
  ...
  it("returns correct days for future trial end", async () => {
    ...
    vi.mocked(prisma.subscription.findUnique).mockResolvedValue(makeSubscription({ trialEnd }));
    const result = await getTrialDaysRemaining("user_123");
