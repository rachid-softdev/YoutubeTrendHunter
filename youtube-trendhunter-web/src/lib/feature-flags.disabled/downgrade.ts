...
export class DowngradeService {
   ...
   */
  async applyDowngrade(orgId: string, targetPlanKey: string): Promise<{
    success: boolean
    ...
    for (const feature of impact.willLoseFeatures) {
      ...
      switch (feature.strategy) {
        ...
        case "FREEZE":
          ...
          console.log(`[DowngradeService] Freeze requested for ${feature.featureKey} on org ${orgId}`)
          affectedFeatures.push(feature.featureKey)
          break
      }
    }
    ...
    return {
      success: true,
      affectedFeatures,
      message: affectedFeatures.length > 0
