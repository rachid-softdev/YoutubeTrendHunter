    ...
    if (dataStart !== -1 && dataEnd !== -1) {
      ...
      for (const price of prices) {
        if (price.id && price.unit_amount) {
          ...
          console.log(
            `Processing price: ${price.id}, amount: ${amount}, tier: ${tier}, interval: ${interval}`,
          );
          ...
          if (amount === 800) {
            if (!priceIds.starterMonthly && !priceIds.starterYearly) {
              if (interval === "month") {
                ...
                console.log(
                  `Found starter monthly price: ${price.id} ($${amount / 100} USD/month)`,
                );
      ...
      if (line.includes("price_")) {
        ...
        if (priceMatch) {
          ...
          for (
            ...
            if (
              checkLine.includes("unit_amount: 800") ||
              (checkLine.includes("800") && checkLine.includes("$8"))
            ) {
              priceIds.starter = priceId;
              console.log(`Found starter price: ${priceId} (8 USD)`);
              break;
            } else if (
              checkLine.includes("unit_amount: 2000") ||
              (checkLine.includes("2000") && checkLine.includes("$20"))
            ) {
