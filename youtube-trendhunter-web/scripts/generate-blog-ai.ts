...
const CATEGORIES: Category[] = [
  { slug: "tendances", name: "Tendances", description: "Analyses des tendances YouTube actuelles", color: "#FF0000" },
  { slug: "guides", name: "Guides", description: "Tutoriels et guides pratiques", color: "#065FD4" },
  { slug: "comparatifs", name: "Comparatifs", description: "Comparatifs d'outils et stratégies", color: "#FFD600" },
  { slug: "analyses", name: "Analyses", description: "Analyses approfondies du marché", color: "#2BA640" },
  { slug: "actualites", name: "Actualités", description: "Nouveautés et mises à jour", color: "#6A0DAD" },
]
...
Titre de l'article: ${topic.title}`
      ...
      const response = await anthropic.messages.create({
        ...
        max_tokens: 4000,
        temperature: 0.7,
        system: "Tu es un expert en création de contenu blog. Tu génères uniquement du JSON valide, sans texte additionnel.",
        messages: [
          {
            role: "user",
            content: prompt,
...
async function main(): Promise<void> {
  ...
  const articlesPerRun = Math.min(
    parseInt(process.env.ARTICLES_PER_RUN || "3", 10),
    5
  )
