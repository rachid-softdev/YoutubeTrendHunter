...
function checkStripeCLI() {
  try {
    ...
    console.log(
      "  Linux: curl -s https://packages.stripe.com/api/security/keypairs/stripe-cli-gpg/public.key | gpg --dearmor | sudo tee /usr/share/keyrings/stripe.gpg",
    );
    console.log(
      '         echo "deb [signed-by=/usr/share/keyrings/stripe.gpg] https://packages.stripe.com/stripe-cli-debian/ stable main" | sudo tee -a /etc/apt/sources.list.d/stripe.list',
    );
